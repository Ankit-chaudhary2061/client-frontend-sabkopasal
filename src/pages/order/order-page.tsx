...
import { fetchMyorder,  updateOrderStatusInStore } from "../../lib/store/checkout/checkout-slice"
import { Link } from "react-router-dom"
import { OrderStatus } from "../../lib/types/checkout-type"
import { socket } from "../../App"

const OrderPage = ()=>{
    const dispatch = useAppDispatch()
    const{myorders}=useAppSelector((store)=>store.order)
    const[selectedItem, setSelecetedItem] = useState<OrderStatus>(OrderStatus.All)
            ...
            <div className="my-2 flex sm:flex-row flex-col">
                <div className="flex flex-row mb-1 sm:mb-0">
                    ...
                    <div className="relative">
                        <select
                        onChange={(e)=>setSelecetedItem(e.target.value as OrderStatus)}
      ...
      return (
        <tr key={orders.id}>
          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
            <Link to={`/myorders/${orders.id}`}>
              <p className="text-blue-900 whitespace-nowrap underline">
                {orders.id}
              </p>
            </Link>
          </td>
